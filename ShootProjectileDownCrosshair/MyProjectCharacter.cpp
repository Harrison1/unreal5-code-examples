...
void AMyProjectCharacter::OnFire()
{
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			...
			/** 
			* projectile code taken/influenced directly from Shooter Game Public/Weapons/ShooterWeapon_Projectile.cpp. 
			* the Shooter Game has better well rounded code production ready code. My code is simplied for learning.
			* link: https://www.unrealengine.com/marketplace/en-US/item/fea72c38b29841e592f9ed2007a9d756 
			*/
			...
			const FVector EndLocation = StartLocation + ShootDir * ProjectileRange;
			...
			if (Hit.bBlockingHit)
			{
				FVector AdjustedDir = (Hit.ImpactPoint - Origin).GetSafeNormal();
				const float DirectionDot = FVector::DotProduct(AdjustedDir, ShootDir);

				if (DirectionDot > 0.5f)
				{
					FireProjectile(Origin, AdjustedDir);
				}
			}
			else
			{
...
// credit to https://www.unrealengine.com/marketplace/en-US/item/fea72c38b29841e592f9ed2007a9d756
void AMyProjectCharacter::FireProjectile(FVector Origin, FVector_NetQuantizeNormal ShootDir)
{
	FTransform SpawnTM(ShootDir.Rotation(), Origin);

	// setup the projectile before spawning it
	AMyProjectProjectile* Projectile = Cast<AMyProjectProjectile>(UGameplayStatics::BeginDeferredActorSpawnFromClass(this, ProjectileClass, SpawnTM));
	if (Projectile)
	{
