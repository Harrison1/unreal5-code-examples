...
{
	/* Fire Instant Shot Line Trace */
	
	// line trace for local player will start from camera
	FVector StartLocation = FirstPersonCameraComponent->GetComponentLocation();
	const FVector AdjustedAim = GetAdjustedAim();

	// Where the line trace should end. I hard coded the distance of 20000, it can be whatever you want, it should typically be a variable
	const FVector EndLocation = StartLocation + AdjustedAim * 20000;
	...
	if (Hit.bBlockingHit)
	{
		// draw a debug line starting from the gun's muzzle
		FVector MuzzleLocation = FP_MuzzleLocation->GetComponentLocation();
		DrawDebugLine(GetWorld(), MuzzleLocation, Hit.Location, FColor::Green, false, 2.0f, 0U, 0.0f);
		
		// credit to Tom Looman, very helpful debug video https://www.tomlooman.com/unreal-engine-4-c-tutorial-effective-debug-logging/
		DrawDebugString(GetWorld(), Hit.ImpactPoint, "Hit", nullptr, FColor::White, 2.0f);
	}
	else 
	{
		// draw a debug line to the end of the line trace
		DrawDebugLine(GetWorld(), FP_MuzzleLocation->GetComponentLocation(), EndLocation, FColor::Green, false, 2.0f, 0U, 0.0f);
	}
...
// credit to https://www.unrealengine.com/marketplace/en-US/item/fea72c38b29841e592f9ed2007a9d756
FVector AMyProjectCharacter::GetAdjustedAim() const
{
	...
	{
		FinalAim = FirstPersonCameraComponent->GetForwardVector();
	}
